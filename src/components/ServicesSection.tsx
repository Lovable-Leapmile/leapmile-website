...
import { ShoppingCart, Package, Store, Wrench, Eye, ArrowRight, CheckCircle } from "lucide-react";
...
const ServicesSection = () => {
  ...
  return <section id="industries" className="py-20 bg-accent/30">
            ...
            {industries.map((industry, index) => <Card key={index} className="relative overflow-hidden transition-all duration-300 group cursor-pointer" onClick={() => navigate(industry.link)}>
                ...
                <CardContent>
                  ...
                  <div className="space-y-2 mb-6">
                    {industry.features.map((feature, featureIndex) => <div key={featureIndex} className="flex items-center text-sm">
                        <CheckCircle className="h-4 w-4 text-primary mr-2 flex-shrink-0" />
                        <span className="text-muted-foreground">{feature}</span>
                      </div>)}
                  </div>
                  
                  <Button variant="outline" className="group/btn hover:bg-primary hover:text-white transition-colors w-full">
                    Learn More
